				     ...
				     bool is_need_fmt)
{
	...
	if (!dai_node) {
		dev_err(card->dev, "%s() Can not get child: dais\n",
			__func__);
		printk("fail %s() Can not get child: dais\n",__func__);
		return -EINVAL;
	}
	...
	for_each_child_of_node(dai_node, child_dai_node) {
		if (is_need_fmt) {
			card->dai_link[dai_num].dai_fmt =snd_soc_of_parse_daifmt(child_dai_node, NULL,NULL,NULL);
			if ((card->dai_link[dai_num].dai_fmt &
				SND_SOC_DAIFMT_MASTER_MASK) == 0) {
				dev_err(card->dev,
					"fail Property 'format' missing or invalid\n");
				return -EINVAL;
			}
		}
		...
		if (!card->dai_link[dai_num].codec_of_node) {
			dev_err(card->dev,
				"fail  Property 'audio-codec' missing or invalid\n");
			printk("Property 'audio-codec' missing or invalid\n");
			return -EINVAL;
		}
		...
		if (!card->dai_link[dai_num].cpu_of_node) {
			dev_err(card->dev,
				"fail Property 'audio-controller' missing or invalid\n");
			printk("Property 'audio-controller' missing or invalid\n");
			return -EINVAL;
		}
	...
	if (dai_num < card->num_links) {
		...
		printk("fail %s() Can not get enough property for dais, dai: %d, max dai num: %d\n",
			__func__, dai_num, card->num_links);
		return -EINVAL;
	}
