...
/**
 *  icm42680_update_period() - Update chip internal period estimation
 *
 *  @st:		driver state
 ...
 */
static void icm42680_update_period(struct icm42680_data *st,
...
/**
 *  icm42680_get_timestamp() - Return the current data timestamp
 *
 *  @st:		driver state
 ...
 */
static s64 icm42680_get_timestamp(struct icm42680_data *st)
{
	s64 ts;
...
{
	int ret;
	struct icm42680_data  *st = iio_priv(indio_dev);
	...
	ret = icm42680_mreg_single_write(st, REG_TMST_CONFIG1_MREG_TOP1,
	...
	ret = icm42680_mreg_single_write(st, REG_FIFO_CONFIG5_MREG_TOP1,
...
/**
 * icm42680_read_fifo() - Transfer data from hardware FIFO to KFIFO.
 */
irqreturn_t icm42680_read_fifo(int irq, void *p)
{
	...
	u8 data[ICM42680_OUTPUT_DATA_SIZE_PULS_ONE], i;
	u16 fifo_count;
	u32 data_len;
	s64 timestamp = 0;
	...
	bytes_per_datum = ICM42680_FIFO_DATUM;
	result = regmap_bulk_read(st->regmap, REG_FIFO_BYTE_COUNT1,
				  data, ICM42680_FIFO_COUNT_BYTE);
	if (result) {
	...
	if (fifo_count > ICM42680_FIFO_COUNT_LIMIT) {
		dev_warn(dev, "fifo overflow reset, cnt: %u\n", fifo_count);
		goto flush_fifo;
	}

	icm42680_update_period(st, pf->timestamp, fifo_count);
	...
	for (i = 0; i < fifo_count; ++i) {
		...
		timestamp = icm42680_get_timestamp(st);
