...
/**
 *  icm42680_set_enable() - enable chip functions.
...
int icm42680_set_enable(struct iio_dev *indio_dev, bool enable)
{
	...
	if (enable) {
		...
		ret = icm42680_reset_fifo(indio_dev);
		if (ret)
			goto error_gyro_off;
		...
		ret = icm42680_set_mode(data, ICM42680_GYRO, false);
		if (ret)
			goto error_gyro_off;

		ret = icm42680_set_mode(data, ICM42680_ACCEL, false);
		if (ret)
			goto error_accl_off;
	}
						  ...
						  bool state)
{
	...
	result = icm42680_set_enable(indio_dev, state);
