...
static int spidev_mst_wait_for_slave_ready(struct spidev_rkmst_data *spidev,
					   u32 timeout_us)
{
	int timeout_count = timeout_us / 100 + 1;

	while (timeout_count--) {
		if (spidev_mst_check_slave_ready(spidev))
			return true;
		else
...
static int spidev_rkmst_probe(struct spi_device *spi)
{
	...
	if (IS_ERR(spidev->ready)) {
		dev_err(&spi->dev, "invalid ready-gpios property in node\n");
		return PTR_ERR(spidev->ready);
	}
