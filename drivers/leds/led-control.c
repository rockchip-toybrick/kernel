...
static int uhrst_probe(struct platform_device *pdev)
{
	...
	ddata->uhrst_gpio = of_get_named_gpio_flags(node, "led-gpio", 0, &flag);
	if (uhrst_gpio < 0) {
		printk("%s() Can not read property led_gpio\n", __FUNCTION__);
		ret = -EIO;
		goto fail0;
	}
	uhrst_gpio = ddata->uhrst_gpio;

	ret = devm_gpio_request(dev, uhrst_gpio, "led-gpio");
	if (ret != 0) {
		printk("Request gpio : %d fail\n", uhrst_gpio);
		return ret;
	}

	gpio_direction_output(uhrst_gpio, GPIO_LOW);
