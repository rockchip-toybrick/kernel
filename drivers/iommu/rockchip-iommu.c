...
#define DTE_ADDR_DUMMY		0xCAFEBABEU
...
#define RK_MMU_STATUS_REPLAY_BUFFER_EMPTY  (1U << 4)
#define RK_MMU_STATUS_PAGE_FAULT_IS_WRITE  (1U << 5)
#define RK_MMU_STATUS_STALL_NOT_ACTIVE     (1U << 31)
...
#define RK_MMU_IRQ_PAGE_FAULT    (1U << 0)  /* page fault */
#define RK_MMU_IRQ_BUS_ERROR     (1U << 1)  /* bus read error */
#define RK_MMU_IRQ_MASK          (RK_MMU_IRQ_PAGE_FAULT | RK_MMU_IRQ_BUS_ERROR)

#define NUM_DT_ENTRIES 1024
#define NUM_PT_ENTRIES 1024

#define SPAGE_ORDER 12U
#define SPAGE_SIZE (1UL << SPAGE_ORDER)

#define DISABLE_FETCH_DTE_TIME_LIMIT (1UL << 31)
...
#define DTE_HI_SHIFT1	24U /* shift bit 8 to bit 32 */
#define DTE_HI_SHIFT2	32U /* shift bit 4 to bit 36 */
...
{
	return (dte & RK_DTE_PT_VALID) != 0U;
}

static inline u32 rk_mk_dte(dma_addr_t pt_dma)
{
	return (u32)((pt_dma & RK_DTE_PT_ADDRESS_MASK) | RK_DTE_PT_VALID);
}
...
static inline u32 rk_mk_dte_v2(dma_addr_t pt_dma)
{
	...
	return (u32)((pt_dma & RK_DTE_PT_ADDRESS_MASK_V2) | RK_DTE_PT_VALID);
}
...
#define RK_PTE_PAGE_FLAGS_MASK    0x000001feU
...
{
	return (pte & RK_PTE_PAGE_VALID) != 0U;
}

#define RK_PTE_PAGE_REPRESENT	(1U << 3)

static inline bool rk_pte_is_page_represent(u32 pte)
{
	return (pte & RK_PTE_PAGE_REPRESENT) != 0U;
}
...
{
	u32 flags = 0;
	unsigned int prot_tmp = (unsigned int)prot;
	u32 pte = (u32)page;

	flags |= ((prot_tmp & IOMMU_READ) != 0U) ? RK_PTE_PAGE_READABLE : 0U;
	flags |= ((prot_tmp & IOMMU_WRITE) != 0U) ? RK_PTE_PAGE_WRITABLE : 0U;
	flags |= ((prot_tmp & IOMMU_PRIV) != 0U) ? RK_PTE_PAGE_REPRESENT : 0U;
	pte &= RK_PTE_PAGE_ADDRESS_MASK;
	return pte | flags | RK_PTE_PAGE_VALID;
}
 ...
 */
#define RK_IOVA_DTE_MASK    0xffc00000U
#define RK_IOVA_DTE_SHIFT   22
#define RK_IOVA_PTE_MASK    0x003ff000U
#define RK_IOVA_PTE_SHIFT   12
#define RK_IOVA_PAGE_MASK   0x00000fffU
			       ...
			       size_t size)
{
	...
	for (i = 0; i < iommu->num_mmu; i++) {
		...
		for (iova = iova_start; iova < iova_end; iova += SPAGE_SIZE) {
			rk_iommu_write(iommu->bases[i], RK_MMU_ZAP_ONE_LINE, (unsigned int)iova);
		}
	}
}
...
static bool rk_iommu_is_stall_active(struct rk_iommu *iommu)
{
	...
	for (i = 0; i < iommu->num_mmu; i++) {
		if ((rk_iommu_read(iommu->bases[i], RK_MMU_STATUS) & RK_MMU_STATUS_STALL_ACTIVE) == 0U) {
			active = (bool)false;
			break;
		}
	}
...
static bool rk_iommu_is_paging_enabled(struct rk_iommu *iommu)
{
	...
	for (i = 0; i < iommu->num_mmu; i++) {
		if ((rk_iommu_read(iommu->bases[i], RK_MMU_STATUS) & RK_MMU_STATUS_PAGING_ENABLED) == 0U) {
			enable = (bool)false;
			break;
		}
	}
...
static bool rk_iommu_is_reset_done(struct rk_iommu *iommu)
{
	...
	for (i = 0; i < iommu->num_mmu; i++) {
		if (rk_iommu_read(iommu->bases[i], RK_MMU_DTE_ADDR) != 0U) {
			done = (bool)false;
			break;
		}
	}
...
static int rk_iommu_force_reset(struct rk_iommu *iommu)
{
	 ...
	 */
	for (i = 0; i < iommu->num_mmu; i++) {
		dte_address_mask = (u32)rk_ops->pt_address(DTE_ADDR_DUMMY);
...
{
	return (u32)((dt_dma & RK_DTE_PT_ADDRESS_MASK) |
...
static void log_iova(struct rk_iommu *iommu, int index, dma_addr_t iova)
{
	...
	u64 dte_index, pte_idx, page_offset;
	u32 mmu_dte_addr;
	...
	pte_idx = rk_iova_pte_index(iova);
	page_offset = rk_iova_page_offset(iova);

	mmu_dte_addr = rk_iommu_read(base, RK_MMU_DTE_ADDR);
	mmu_dte_addr_phys = rk_ops->dte_addr_phys(mmu_dte_addr);

	dte_addr_phys = mmu_dte_addr_phys + (4U * dte_index);
	...
	pte_addr_phys = rk_ops->pt_address(dte) + (pte_idx * 4U);
...
print_it:
	dev_err(iommu->dev, "iova = %pad: dte_index: %#03x pte_idx: %#03x page_offset: %#03x\n",
		&iova, dte_index, pte_idx, page_offset);
...
static int rk_pagefault_done(struct rk_iommu *iommu)
{
	...
	for (i = 0; i < iommu->num_mmu; i++) {
		...
		if ((int_status & RK_MMU_IRQ_PAGE_FAULT) != 0U) {
			int flags;

			status = rk_iommu_read(iommu->bases[i], RK_MMU_STATUS);
			flags = ((status & RK_MMU_STATUS_PAGE_FAULT_IS_WRITE) != 0U) ?
			...
			if (!iommu->master_handle_irq) {
				 ...
				 */
				if (iommu->domain) {
					report_status = report_iommu_fault(iommu->domain, iommu->dev, iova,
									   (int)status);
					if (report_status != 0) {
						dev_err(iommu->dev, "report_iommu_fault error, %d\n", report_status);
					}
				} else {
		...
		if ((int_status & RK_MMU_IRQ_BUS_ERROR) != 0U) {
			dev_err(iommu->dev, "BUS_ERROR occurred at %pad\n", &iova);
		}

		if ((int_status & ~RK_MMU_IRQ_MASK) != 0U)  {
...
static irqreturn_t rk_iommu_irq(int irq, void *dev_id)
{
	...
	if (iommu->master_handle_irq) {
		if (iommu->domain) {
			report_status = report_iommu_fault(iommu->domain, iommu->dev, 0xffffffffUL, 0x0);
		}
	} else {
		report_status = rk_pagefault_done(iommu);
	}
				  ...
				  dma_addr_t iova)
{
	...
	if (dma_mapping_error(dma_dev, pt_dma) != 0) {
				  ...
				  size_t size, struct rk_iommu *iommu)
{
	...
	unsigned int pte_total = (unsigned int)(size / SPAGE_SIZE);
	unsigned int prot = IOMMU_READ | IOMMU_WRITE | IOMMU_PRIV;
			     ...
			     phys_addr_t paddr, size_t size, int prot)
{
	...
	for (pte_count = 0; pte_count < pte_total; pte_count++) {
		...
		if (((unsigned int)prot & IOMMU_PRIV) != 0U) {
...
unwind:
	...
	(void)rk_iommu_unmap_iova(rk_domain, pte_addr, pte_dma,
			    pte_count * SPAGE_SIZE, NULL);

	iova += pte_count * SPAGE_SIZE;
	page_phys = rk_ops->pt_address(pte_addr[pte_count]);
	(void)pr_err("iova: %pad already mapped to %pa cannot remap to phys: %pa prot: %#x\n",
			...
			phys_addr_t paddr, size_t size, int prot, gfp_t gfp)
{
	...
	if (IS_ERR(page_table)) {
		spin_unlock_irqrestore(&rk_domain->dt_lock, flags);
		return (int)PTR_ERR(page_table);
	}

	dte = rk_domain->dt[rk_iova_dte_index(iova)];
	pte_idx = rk_iova_pte_index(iova);
	pte_addr = &page_table[pte_idx];
	pte_dma = rk_ops->pt_address(dte) + pte_idx * sizeof(u32);
...
static void rk_iommu_disable(struct rk_iommu *iommu)
{
	...
	WARN_ON(clk_bulk_enable(iommu->num_clocks, iommu->clocks));
	(void)rk_iommu_enable_stall(iommu);
	(void)rk_iommu_disable_paging(iommu);
	for (i = 0; i < iommu->num_mmu; i++) {
		rk_iommu_write(iommu->bases[i], RK_MMU_INT_MASK, 0);
		rk_iommu_write(iommu->bases[i], RK_MMU_DTE_ADDR, 0);
	}
	(void)rk_iommu_disable_stall(iommu);
	clk_bulk_disable(iommu->num_clocks, iommu->clocks);

	iommu->iommu_enabled = (bool)false;
}
...
static int rk_iommu_enable(struct rk_iommu *iommu)
{
	...
	for (i = 0; i < iommu->num_mmu; i++) {
		...
		auto_gate |= (unsigned int)DISABLE_FETCH_DTE_TIME_LIMIT;
...
static struct iommu_domain *rk_iommu_domain_alloc(unsigned type)
{
	...
	if (type == IOMMU_DOMAIN_DMA) {
		dma_cookie_ret = iommu_get_dma_cookie(&rk_domain->domain);
		if (dma_cookie_ret != 0) {
	...
	if (dma_mapping_error(dma_dev, rk_domain->dt_dma) != 0) {
	...
	rk_domain->domain.geometry.force_aperture = (bool)true;
...
static struct iommu_device *rk_iommu_probe_device(struct device *dev)
{
	...
	data->link = device_link_add(dev, iommu->dev,
				     (unsigned int)(DL_FLAG_STATELESS | DL_FLAG_PM_RUNTIME));

	data->defer_attach = (bool)false;
	...
	(void)dma_set_max_seg_size(dev, DMA_BIT_MASK(32));
			     ...
			     struct of_phandle_args *args)
{
	...
	if (strncmp(dev_name(dev), "vop", 3) == 0) {
		data->defer_attach = (bool)true;
	}
...
static int rk_iommu_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < num_res; i++) {
		res = platform_get_resource(pdev, IORESOURCE_MEM, (unsigned int)i);
		if (!res) {
			continue;
		}
	...
	if (iommu->num_mmu == 0) {
		return (int)PTR_ERR(iommu->bases[0]);
	}
	...
	(void)bus_set_iommu(&platform_bus_type, &rk_iommu_ops);
	...
	for (i = 0; i < (unsigned int)iommu->num_irq; i++) {
		...
		err = devm_request_irq(iommu->dev, (unsigned int)irq, rk_iommu_irq,
...
skip_request_irq:
	if ((res_page == 0ULL) && iommu->need_res_map) {
		res_page =__pa_symbol(reserve_range);

		(void)pr_info("%s,%d, res_page = 0x%pa\n", __func__, __LINE__, &res_page);
	}

	(void)dma_set_mask_and_coherent(dev, rk_ops->dma_bit_mask);
...
static void rk_iommu_shutdown(struct platform_device *pdev)
{
	...
	for (i = 0; i < iommu->num_irq; i++) {
		int irq = platform_get_irq(pdev, (unsigned int)i);

		devm_free_irq(iommu->dev, (unsigned int)irq, iommu);
	}

skip_free_irq:
	if (!iommu->dlr_disable) {
		(void)pm_runtime_force_suspend(&pdev->dev);
	}
}
